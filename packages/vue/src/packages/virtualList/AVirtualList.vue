...
defineSlots<{
  default?: (scope: { item: VirtualListItem<T> }) => unknown;
}>();
...
const scrollToItem = (idOrFunc: string | ((item: VirtualListItem<T>) => boolean)) => {
  const id = typeof idOrFunc === 'function' ? transformedItems.find(idOrFunc)?.id : idOrFunc;
  if (!id) {
    return;
  }
  // Accumulate item heights until the target id is found.
  let height = 0;
